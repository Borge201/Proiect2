...
char get_Curent_state(void)
 {    //citim cei 6 biti de la dsp pentru valoarea curentului
  ...
  val=PINB & 0b11000000;    //schimba in port C
  val=val<<2;
  val=2>>val;
...
LEDstate=get_Curent_state();
 if(LEDstate==0b11111110){   //schimba in port D2, D3, D4
 PORTB=0b11111110;
 }else if(LEDstate==0b11111101){
 PORTB=0b11111101;
 }else if(LEDstate==0b11111011){
 PORTB=0b11111011;
 }
 else {
 PORTB=0xFF;// nu ne afiseaza niciun led
 }
//aceasta functie preia valoarea trimisa de dsp( valorile sunt intre anumite nivele
//exemplu nivel 0 (LED0) corespunde unei valori intre (10mA-100mA) s.s.m.d. 
}
/*
...
while (1)
      {
      LED_Stare_Curent();
      if(check_pulse==0 ) //fal portC.0
      {
        cont_pulse=0;
        while(check_pulse!=1) // fal port C.1
          {if(in_semnal_p==0) // DSP ar trebui sa trimita semnal de genul 1110111 
            {
